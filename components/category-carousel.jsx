    ...
    <section className="section" id="categories">
      <div className="container">
        <div className="section-title carousel-title">
          <div><span className="eyebrow">Shop by category</span><h2>Browse your creative shelf</h2></div>
          <div className="carousel-actions">
            <button type="button" onClick={() => scroll(-1)} aria-label="Previous categories"><ArrowLeft size={17} /></button>
            <button type="button" onClick={() => scroll(1)} aria-label="Next categories"><ArrowRight size={17} /></button>
          ...
          {visible.map((category) => (
            <Link href={`/category/${category.slug}`} className="category-pill" key={category.id}>
              <span className="circle">
                {category.image?.src ? <WpImage src={category.image.src} alt={category.name} /> : <span className="category-fallback">{category.name.slice(0, 1)}</span>}
              </span>
