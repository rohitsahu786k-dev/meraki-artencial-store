...
export function CollectionBrowser({ products, categories, attributes, basePath, activeQuery = {} }) {
  ...
  function toggleHref(key, value) {
    const currentValue = Array.isArray(activeQuery[key]) ? activeQuery[key][0] : activeQuery[key];
    return withQuery(basePath, activeQuery, { [key]: currentValue === value ? null : value, page: null });
  }

  function priceHref(min, max) {
    const currentMin = Array.isArray(activeQuery.min_price) ? activeQuery.min_price[0] : activeQuery.min_price;
    const currentMax = Array.isArray(activeQuery.max_price) ? activeQuery.max_price[0] : activeQuery.max_price;
    const selected = currentMin === min && currentMax === max;
    ...
    <aside className="filter-panel myntra-filter">
      ...
      {attributes.map((attribute) => (
        <details open={attribute.taxonomy === "pa_color"} key={attribute.id}>
          <summary>{decodeHtml(attribute.name)} <ChevronDown size={15} /></summary>
          <div className="filter-options">
            {attribute.terms.filter((term) => term.count > 0).slice(0, 28).map((term) => {
      ...
      <div className="collection-toolbar">
        <div className="toolbar-left">
          ...
          <div className="sort-select-wrapper">
            ...
            <select id="collection-sort-select" className="collection-sort-dropdown" value={currentSort} onChange={handleSortChange}>
