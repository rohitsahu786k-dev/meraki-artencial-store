...
export function CollectionBrowser({ products, categories, attributes, basePath, activeQuery = {} }) {
  ...
  const filters = (
    <aside className="filter-panel iconic-filter">
      ...
      {activeCount ? <Link className="clear-filters" href={basePath} onClick={() => setFiltersOpen(false)}>Clear all filters</Link> : null}
      <details open>
        <summary>Category <ChevronDown size={15} /></summary>
        {visibleCategories.slice(0, 28).map((category) => (
          <Link href={`/category/${category.slug}`} key={category.id} onClick={() => setFiltersOpen(false)}><span className="filter-check" /><span>{category.name}</span><small>({category.count})</small></Link>
        ))}
      </details>
      <details>
      ...
      <details>
        ...
        <Link href={priceHref(null, "50000")}><span className="filter-check">{!activeQuery.min_price && activeQuery.max_price === "50000" ? <Check size={11} /> : null}</span><span>Under Rs. 500</span></Link>
        <Link href={priceHref("50000", "100000")}><span className="filter-check">{activeQuery.min_price === "50000" && activeQuery.max_price === "100000" ? <Check size={11} /> : null}</span><span>Rs. 500 - Rs. 1,000</span></Link>
        <Link href={priceHref("100000", null)}><span className="filter-check">{activeQuery.min_price === "100000" && !activeQuery.max_price ? <Check size={11} /> : null}</span><span>Above Rs. 1,000</span></Link>
      </details>
      {attributes.map((attribute) => (
        <details key={attribute.id}>
          <summary>{attribute.name} <ChevronDown size={15} /></summary>
          {attribute.terms.filter((term) => term.count > 0).map((term) => {
            ...
            <Link href={toggleHref(key, term.slug)} key={term.id}>
              <span className={attribute.taxonomy === "pa_color" ? `filter-swatch swatch-${term.slug} ${selected ? "selected" : ""}` : "filter-check"}>{selected ? <Check size={11} /> : null}</span>
              <span>{term.name}</span><small>({term.count})</small>
            </Link>
          )})}
        </details>
      ))}
    ...
    <>
      <div className="collection-toolbar">
        ...
        <div className="grid-switcher" aria-label="Grid density">
          <button className={columns === 2 ? "active" : ""} onClick={() => setColumns(2)} aria-label="Two columns"><Grid2X2 size={16} /></button>
          <button className={columns === 3 ? "active" : ""} onClick={() => setColumns(3)} aria-label="Three columns"><LayoutGrid size={16} /></button>
          <button className={columns === 4 ? "active" : ""} onClick={() => setColumns(4)} aria-label="Four columns"><Grid3X3 size={16} /></button>
        </div>
      </div>
      <div className="archive-layout iconic-archive">
        ...
        <div className={`product-grid collection-grid columns-${columns}`}>
          {products.length ? products.map((product) => <ProductCard key={product.id} product={product} />) : <p className="muted">No products found.</p>}
        </div>
      </div>
    </>
  );
