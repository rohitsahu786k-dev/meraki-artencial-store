  ...
  return (
    <div className="checkout-layout checkout-handoff">
      ...
      <aside className="summary-panel premium-summary">
        <h2>Checkout summary</h2><p><span>Items</span><strong>{items.reduce((sum, item) => sum + item.quantity, 0)}</strong></p><p><span>Subtotal</span><strong>{money(subtotal)}</strong></p>{coupon ? <p><span>Coupon</span><strong>{coupon.toUpperCase()}</strong></p> : null}
