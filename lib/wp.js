...
export async function getProduct(slug) {
  ...
  // getProducts already hydrates variations for every product in the list.
...
export async function getRelatedProducts(productId) {
  const products = await wpFetch(`/wp-json/wc/store/v1/products?related=${productId}&per_page=8`).catch(() => []);
  return hydrateProductList(Array.isArray(products) ? products : []);
}
