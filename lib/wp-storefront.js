import { getFrontPage, getMedia, getPage, getPages } from "@/lib/wp";
...
export async function getAnnouncementBar() {
  ...
  if (frontPage?.acf?.top_announcement) {
    return {
      ...
      href: frontPage.acf.top_announcement_link || "/shop",
    };
  }
...
export async function getManagedHeroBanners(products = []) {
  // 1. Check WordPress ACF Fields on Front Page (Repeater or Individual Fields)
  const frontPage = await getFrontPage().catch(() => null);
  ...
  if (acf) {
    ...
    // 1b. Individual fields format: desktop_banner_1, desktop_banner_2, etc.
