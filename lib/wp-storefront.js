...
export async function getStoreData() {
  const customData = await wpFetch("/wp-json/meraki/v1/store-data", storefrontFreshness).catch(() => null);
  if (customData?.banners || customData?.announcement || customData?.homepage_categories) {
    return customData;
  }
  return null;
}

export async function getHomepageCategorySlugs() {
  const storeData = await getStoreData().catch(() => null);
  if (Array.isArray(storeData?.homepage_categories) && storeData.homepage_categories.length > 0) {
    return storeData.homepage_categories.filter(Boolean);
  }
  return [];
}

export async function getPageBanner(slug) {
  const storeData = await getStoreData().catch(() => null);
  return storeData?.pages_banners?.[slug] || null;
}
...
export async function getManagedHeroBanners(products = []) {
  ...
  const cptBanners = await wpFetch("/wp-json/wp/v2/banners?per_page=20&orderby=menu_order&order=asc&_embed=1", storefrontFreshness).catch(() => []);
